...
import supported from '../supported.json' with {
    type: 'json',
};
...
async function getFindUp() {
    if (globalThis.MADRUN_FINDUP)
        return globalThis.MADRUN_FINDUP;
...
export const cutEnv = async (names, opts = '', env, scripts) => {
    names = maybeArray(names);
    scripts = scripts || await getScripts();
    
    const isParallel = false;
    
    const commands = await runNames(names, opts, scripts, {
        env,
        cutEnv: true,
        isParallel,
    });
...
export const run = async (name, opts = '', env, scripts) => {
    scripts = scripts || await getScripts();
...
export async function series(names, opts = '', env, scripts) {
    names = maybeArray(names);
    scripts = scripts || await getScripts();
...
export const parallel = async (names, opts = '', env, scripts) => {
    scripts = scripts || await getScripts();
    names = maybeArray(names);
...
async function runNames(names, opts, scripts, {env, isParallel, cutEnv}) {
    const cmds = [];
